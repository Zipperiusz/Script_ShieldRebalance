...
void castSpell(MagicParadeInfo magicParadeInfo) {
    Entity Defender = magicParadeInfo.entity;
    if (!Defender.NPC.IsValid())return;
    Entity Target = Defender.NPC.GetCurrentTarget();
    bCMatrix Pose = Defender.GetPose();
    Pose.AccessTranslation().AccessY() += 130;
    Entity Spell = Template(magicParadeInfo.spell);
